...
describe('obj exports', () => {
    describe('has', () => {
        it('should return true for keys that is contained in myObj', () => {
            ...
            let myObj = {a: 1} as looseType;
            let batcher = createObjectBatcher(myObj);

            expect(_objHas(batcher, '')).toEqual(false);
            expect(_objHas(batcher, 'b')).toEqual(false);
            expect(_objHas(batcher, 'a')).toEqual(true);
        })
    ...
    describe('clear', () => {
        ...
        it('myMap should be batcher currentValue', () => {
            let myMap = {};
            let batcher = createObjectBatcher(myMap);
    ...
    describe('spread', () => {
        it('should pass keys and values copyObj to batcher current value, without change myObj to another spread', () => {
            let myObj: {a: number, b: number, c: number, d: number, e: number} = {a: 1, b: 2, c: 3, d: 4, e: 5};
            let batcher = createObjectBatcher(myObj);
            let copyObjs: Batcher<typeof myObj>[] = [createObjectBatcher({a: 0, b: 0, c: 0, d: 0, e: 0})];
